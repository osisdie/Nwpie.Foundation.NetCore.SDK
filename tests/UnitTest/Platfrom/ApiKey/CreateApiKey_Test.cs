...
{
    public class CreateApiKey_Test : TestBase
    {
        public CreateApiKey_Test(ITestOutputHelper output) : base(output) { }
        ...
        public async Task ApiKey_New_Test()
        {
            var sb = new StringBuilder();
            sb.Append($"Created at: {DateTime.Now:s}{Environment.NewLine}");
            var apiKeyPrefix = $"{CommonConst.SdkPrefix.ToLower()}.kvs";
            var note = "HP Demo";
        ...
        public override Task<bool> IsReady()
        {
            ...
            {
                //m_RemoteConfigKeyClient = ComponentMgr.Instance
                //    .TryResolve<IClient_ApiKey>();
            }
